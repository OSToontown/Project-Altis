...
class Toon(Avatar.Avatar, ToonHead):
    ...
    def putOnSuit(self, suitType, setDisplayName = True, rental = False):
        self.notify.debug('Putting on Suit (Disguise)!')
        self.notify.debug('Suit Type is %s' % (str(suitType)))
        if self.isDisguised:
            self.takeOffSuit()
        ...
        if rental == True:
            if SuitDNA.suitDepts[deptIndex] == 's':
                ...
                suitType = 'cc'
        self.notify.debug('Setting Suit Style!')
        dna.newSuit(suitType)
        suit.setStyle(dna)
        suit.isDisguised = 1
        self.notify.debug('Generating Suit!')
        suit.generateSuit()
        ...
        Emote.globalEmote.disableAll(self)
        self.notify.debug('Hiding Toon Geom!')
        ...
        self.notify.debug('Setting Toon Head Position!')
        ...
        self.notify.debug('Getting and Setting Suit Geom!')
        suitGeom = suit.getGeomNode()
        suitGeom.reparentTo(self)
        self.notify.debug('Checking if Suit (Disguise) is Rental!')
        if rental == True:
            suit.makeRentalSuit(SuitDNA.suitDepts[deptIndex])
        self.suit = suit
        self.suitGeom = suitGeom
        self.setHeight(suit.getHeight())
        self.notify.debug('Setting Suit (Disguise) nametag position!')
        self.nametag3d.setPos(0, 0, self.height + 1.3)
        if self.isLocal():
            if hasattr(self, 'book'):
                self.book.obscureButton(1)
            self.notify.debug('Setting Suit (Disguise) walk and rotation speed!')
