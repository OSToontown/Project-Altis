...
class ChatLog(DirectFrame, DirectObject):
    ...
    def addToLog(self, msg):
        msg = DirectLabel(relief = None, text = msg, text_scale = 0.03, text_align = TextNode.ACenter, text_wordwrap = 20)
        self.log.addItem(msg)
        self.log.scrollTo(len(self.log['items']) - 1)
        
    def toggle(self):
        if self.isHidden:
            self.toggleBtn.scaleInterval(0.5, Vec3(-1, 1, 1)).start()
            self.posInterval(0.5, Point3(0.12, 0, 0.2)).start()
            self.isHidden = False
            base.setCellsActive([base.leftCells[0]], 0)
        else:
            self.toggleBtn.scaleInterval(0.5, Vec3(1, 1, 1)).start()
            self.posInterval(0.5, Point3(-1, 0, 0.2)).start()
            self.isHidden = True
