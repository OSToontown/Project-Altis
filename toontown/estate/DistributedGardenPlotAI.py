...
class DistributedGardenPlotAI(DistributedLawnDecorAI):
    ...
    def plantFlower(self, species, variety, usingGardenFlowerAll=0):
        av = self.__initialSanityCheck(GardenGlobals.FLOWER_TYPE if not usingGardenFlowerAll else None, usingGardenFlowerAll)
        if not av:
            return
        ...
        def _plant(task):
            ...
            self.air.writeServerEvent('plant-flower', self.__plantingAvId, species=species, variety=variety,
                                      plot=self.plot, name=attr.get('name', 'unknown garden flower'))
            if task:
                return task.done
