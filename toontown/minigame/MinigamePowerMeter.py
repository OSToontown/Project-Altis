...
class MinigamePowerMeter(DirectFrame):
    ...
    def __createSpeedGauge(self):
        gaugeA = DirectWaitBar(parent=self, relief=DGG.RAISED, range=self.gaugeSize, frameSize=(-0.6,
         0.6,
         -0.2,
         0.2), borderWidth=(0.02, 0.02), scale=TTLocalizer.MPMgaugeA, pos=(0, 0, 0), frameColor=LVector4f(0.0, 0.0, 0.0, 0.0), barColor=LVector4f(0, 1, 0, 0.6), sortOrder=DGG.FOREGROUND_SORT_INDEX)
        gaugeA.setR(-90)
        gaugeA['value'] = 0
        ...
        gaugeTargetTop = DirectWaitBar(parent=self, relief=DGG.RAISED, range=self.gaugeSize, frameSize=(-0.6,
         0.6,
         -0.2,
         0.2), borderWidth=(0.02, 0.02), scale=TTLocalizer.MPMgaugeTargetTop, pos=(0, 0, 0), frameColor=LVector4f(1, 1, 1, 1), barColor=LVector4f(1, 0, 0, 1), sortOrder=DGG.BACKGROUND_SORT_INDEX + 1)
        ...
        gaugeTargetBot = DirectWaitBar(parent=self, relief=DGG.RAISED, range=self.gaugeSize, frameSize=(-0.6,
         0.6,
         -0.2,
         0.2), borderWidth=(0.02, 0.02), scale=TTLocalizer.MPMgaugeTargetBot, pos=(0, 0, 0), frameColor=LVector4f(1, 1, 1, 0), barColor=LVector4f(1, 1, 1, 1), sortOrder=DGG.BACKGROUND_SORT_INDEX + 2)
