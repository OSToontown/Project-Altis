...
class PurchaseBase(StateData.StateData):
    ...
    def load(self, purchaseModels = None):
        ...
        if settings['newGui'] == True:
            self.pointDisplay = DirectLabel(parent=self.frame, relief=None, pos=(0, 0, -0.5), text=str(self.toon.getMoney()), text_scale=0.2, text_fg=(0.95, 0.95, 0, 1), text_shadow=(0, 0, 0, 1), text_pos=(0, -0.1, 0), image=self.jarImage, text_font=ToontownGlobals.getSignFont())
        else:
