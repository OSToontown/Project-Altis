...
class DistributedBoardOffice(DistributedObject.DistributedObject):
    ...
    ReadyPost = 'BoardOfficeReady'
    WinEvent = 'BoardOfficeWinEvent'
    ...
    def setBoardOfficeId(self, id):
        DistributedBoardOffice.notify.debug('setBoardOfficeId: %s' % id)
        self.boardofficeId = id

    def setFloorNum(self, num):
        DistributedBoardOffice.notify.debug('floorNum: %s' % num)
        self.floorNum = num
    ...
    def setRoomDoIds(self, roomDoIds):
        ...
        self.roomWatcher = BulletinBoardWatcher.BulletinBoardWatcher('roomWatcher-%s' % self.doId, [ DistributedBoardOfficeRoom.getBoardOfficeRoomReadyPostName(doId) for doId in self.roomDoIds ], self.gotAllRooms)

    def gotAllRooms(self):
        self.notify.debug('boardoffice %s: got all rooms' % self.doId)
        if self.roomWatcher:
            self.roomWatcher.destroy()
        self.roomWatcher = None
        self.geom = render.attachNewNode('boardoffice%s' % self.doId)
        for doId in self.roomDoIds:
            self.rooms.append(base.cr.doId2do[doId])
            self.rooms[-1].setBoardOffice(self)

        self.notify.info('boardofficeId %s, floor %s, %s' % (self.boardofficeId, self.floorNum, self.rooms[0].avIdList))
        ...
        for i, room in enumerate(self.rooms):
            ...
            if i < numRooms - 1:
                hallway = BoardOfficeRoom.BoardOfficeRoom(self.layout.getHallwayModel(i))
        ...
        def handleCameraRayFloorCollision(collEntry, self = self):
            ...
            prefix = BoardOfficeRoom.BoardOfficeRoom.FloorCollPrefix
        ...
        if bboard.has('boardofficeRoom'):
            self.warpToRoom(bboard.get('boardofficeRoom'))
        firstSetZoneDoneEvent = self.cr.getNextSetZoneDoneEvent()

        def handleFirstSetZoneDone():
            self.notify.debug('boardofficeHandleFirstSetZoneDone')
            bboard.post(DistributedBoardOffice.ReadyPost, self)
    ...
    def getAllRoomsTimeout(self):
        self.notify.warning('boardoffice %s: timed out waiting for room objs' % self.doId)
    ...
    def setBossConfronted(self, avId):
        ...
        if av is None:
            return
        base.localAvatar.setSystemMessage(avId, TTLocalizer.BoardOfficeBossConfrontedMsg % av.getName())
        return
    ...
    def handleScreenshot(self):
        base.addScreenshotString('boardofficeId: %s, floor (from 1): %s' % (self.boardofficeId, self.floorNum + 1))
