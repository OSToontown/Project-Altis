...
class BoardOfficeRoom(DirectObject.DirectObject):
    FloorCollPrefix = 'mintFloorColl'
    BoardOfficeDoorFrame = 'phase_10/models/cashbotHQ/DoorFrame'

    def __init__(self, path = None):
        if path is not None:
            if path in BoardOfficeRoomSpecs.BoardOfficeConnectorRooms:
                loadFunc = loader.loadModelCopy
            else:
                loadFunc = loader.loadModel
            self.setGeom(loadFunc(path))
        self.localToonFSM = ClassicFSM.ClassicFSM('BoardOfficeRoomLocalToonPresent', [State.State('off', self.enterLtOff, self.exitLtOff, ['notPresent']), State.State('notPresent', self.enterLtNotPresent, self.exitLtNotPresent, ['present']), State.State('present', self.enterLtPresent, self.exitLtPresent, ['notPresent'])], 'notPresent', 'notPresent')
    ...
    def attachTo(self, other, rng):
        ...
        tempNode.removeNode()
        doorFrame = loader.loadModel(BoardOfficeRoom.BoardOfficeDoorFrame)
        doorFrame.reparentTo(thisDoor)

    def getFloorCollName(self):
        return '%s%s' % (BoardOfficeRoom.FloorCollPrefix, self.roomNum)
